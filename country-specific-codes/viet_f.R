...
# remove Notifs_f that were incident before study period
dat <- dat %>%
...
modelConsts <- list(L = nrow(dat), 
                    p = dat$p,  
                    theta = mean(dat$Notifs_f))

modelData <- list(x = dat$Notifs_f)

initsList <- list(lambda = abs(rnorm(nrow(dat), mean(dat$Notifs_f), sd(dat$Notifs_f))))
...
initsList <- list(list(lambda = abs(rnorm(nrow(dat), mean(dat$Notifs_f), sd(dat$Notifs_f)))),
                  list(lambda = abs(rnorm(nrow(dat), mean(dat$Notifs_f), sd(dat$Notifs_f)))),
                  list(lambda = abs(rnorm(nrow(dat), mean(dat$Notifs_f), sd(dat$Notifs_f)))))
...
save(mcmc.out, file = "project1_mcmc/cure_models/remove_extra_notifs/by_sex/output/viet_f.RData")
