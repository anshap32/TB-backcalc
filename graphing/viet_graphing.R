...
pops <- read_excel("country_pops.xlsx") %>% 
  filter(Country == "Viet Nam", year < 2020)

load("project1_mcmc/cure_models/remove_extra_notifs/by_sex/output/viet_all.RData")
L <- nrow(dat)
...
graph_dat2_f <- as.data.frame(cbind(dat$Year, mcmc.out$summary$all.chains[1:24, 2], 
                                    "Estimated Incidence", 
                                    mcmc.out$summary$all.chains[1:24, 4], 
                                    mcmc.out$summary$all.chains[1:24, 5]))
...
graph_dat2_m <- as.data.frame(cbind(dat$Year, mcmc.out$summary$all.chains[25:48, 2], 
                                    "Estimated Incidence", 
                                    mcmc.out$summary$all.chains[25:48, 4], 
                                    mcmc.out$summary$all.chains[25:48, 5]))
...
graph_dat2_all <- as.data.frame(cbind(dat$Year, mcmc.out$summary$all.chains[49:72, 2], 
                                    "Estimated Incidence", 
                                    mcmc.out$summary$all.chains[49:72, 4], 
                                    mcmc.out$summary$all.chains[49:72, 5]))
...
c <- ggplot(graph_all) + 
  ...
  labs(title = "(c)", x = "Year", y = "Count per 100,000 population") + 
  theme_bw()+ 
...
save(c, file = "project1_mcmc/graphing/output/viet_my.rdata")
...
ratio <- as.data.frame(cbind(dat$Year, mcmc.out$summary$all.chains[73:96, 2], 
                             mcmc.out$summary$all.chains[73:96, 4], 
                             mcmc.out$summary$all.chains[73:96, 5]))
...
f
save(f, file = "project1_mcmc/graphing/output/viet_comp.rdata")
