...
load("project1_mcmc/cure_models/remove_extra_notifs/by_sex/output/phl_all.RData")
L <- nrow(dat)
k <- 6.06; r <- 1/.34 # shape; scale
...
graph_dat2_f <- as.data.frame(cbind(dat$Year, mcmc.out$summary$all.chains[1:17, 2], 
                                    "Estimated Incidence", 
                                    mcmc.out$summary$all.chains[1:17, 4], 
                                    mcmc.out$summary$all.chains[1:17, 5]))
...
graph_dat2_m <- as.data.frame(cbind(dat$Year, mcmc.out$summary$all.chains[18:34, 2], 
                                    "Estimated Incidence", 
                                    mcmc.out$summary$all.chains[18:34, 4], 
                                    mcmc.out$summary$all.chains[18:34, 5]))
...
graph_dat2_all <- as.data.frame(cbind(dat$Year, mcmc.out$summary$all.chains[35:51, 2], 
                                    "Estimated Incidence", 
                                    mcmc.out$summary$all.chains[35:51, 4], 
                                    mcmc.out$summary$all.chains[35:51, 5]))
...
b <- ggplot(graph_all) + 
  ...
  labs(title = "(b)", x = "Year", y = "Count per 100,000 population") + 
  theme_bw()+ 
...
b
save(b, file = "project1_mcmc/graphing/output/phl_my.rdata")
...
ratio <- as.data.frame(cbind(dat$Year, mcmc.out$summary$all.chains[52:68, 2], 
                             mcmc.out$summary$all.chains[52:68, 4], 
                             mcmc.out$summary$all.chains[52:68, 5]))
...
e
save(e, file = "project1_mcmc/graphing/output/phl_comp.rdata")
